////
Adapted from clojure.org's "Learn Clojure" guide, content/guides/learn/syntax.adoc.
...
Diagrams from the original page are not reproduced here (see TODOs below) —
see NOTICE.md for full details on what changed.
////
[#ch-2-1-syntax]
= Syntax & the Reader
...
// TODO: redraw diagram — original showed .java source -> javac -> .class bytecode -> JVM.
...
// TODO: redraw diagram — original showed source -> Reader -> Clojure data -> compiler -> JVM bytecode.
In Clojure, source code is read as characters by the https://clojure.org/reference/reader[Reader]. The Reader may read the source either from `.clj` files or be given a series of expressions interactively. The Reader produces Clojure data. The Clojure compiler then produces the bytecode for the JVM.
...
// TODO: redraw diagram — original illustrated (+ 3 4) split into syntax (the list structure, in green) vs semantics (how it's evaluated, in blue).
Most literal Clojure forms evaluate to themselves, *except* symbols and lists. Symbols are used to refer to something else and when evaluated, return what they refer to. Lists are evaluated as invocation.

Consider the expression `(+ 3 4)`: it is read as a list containing the symbol `+` and two numbers (`3` and `4`). The first element (where `+` is found) can be called "function position", that is, a place to find the thing to invoke. While functions are an obvious thing to invoke, there are also a few special operators known to the runtime, macros, and a handful of other invokable things.
...
As you go through this book, please feel free to examine the docstring and source for the functions you are using. Exploring the implementation of the Clojure library itself is an excellent way to learn more about the language and how it is used.
