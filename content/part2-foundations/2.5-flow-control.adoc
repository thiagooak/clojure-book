////
Adapted from clojure.org's "Learn Clojure" guide, content/guides/learn/flow.adoc.
...
Flow control operators are also extensible via macros, which allow the compiler to be extended by user code. We cover macros briefly in <<ch-4-4-macros,Macros>>; for a deeper dive, see https://clojure.org/reference/macros[the Macros reference], https://aphyr.com/posts/305-clojure-from-the-ground-up-macros[Clojure from the Ground Up], or https://www.braveclojure.com/writing-macros/[Clojure for the Brave and True], among many other places.
...
We come back to this in much more depth in <<ch-3-2-error-handling,Error Handling & Reading Stack Traces>>. For now, the basics:
